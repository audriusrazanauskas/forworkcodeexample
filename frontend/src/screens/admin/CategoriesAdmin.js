...
const CategoriesAdmin = () => {
  ...
  const pageSizes = [5, 10, 20, 25, 100];

  const dispatch = useDispatch();

  const categoryList = useSelector((state) => state.categoryList);
  const { loading, error, categories, pages } = categoryList;
  ...
  useDispatch(listCategories(categoryName, page, pageSize, manufacturer));

  const createCategoryHandler = (e) => {
    e.preventDefault();

    dispatch(createCategory({ name, parentId, slug }));
  ...
  const findByCategoryName = () => {
    setPage(1);
    dispatch(listCategories(categoryName));
  };
  ...
  // NEED TO FIX ALL CODE THAT AFTER CATEGORY IS CREATED, IT DOES NOT REFRESH THE TABLE AND SHOW THE NEW CATEGORY
  ...
  useEffect(() => {
    if (successCreate) {
      ...
      dispatch(listCategories(categoryName, page, pageSize, manufacturer));
          ...
          return (
            <>
              <TableButtonsWrapper>
                <TableButton
                  className="edit"
                  onClick={() => navigate(`/admin/categories/${value}/edit`)}
                >
                  <svg
                    width="20"
                    height="20"
    ...
    <>
      <Wrapper>
        <InnerHeader>
          ...
          <InnerHeaderRight>
            <Button onClick={openCategoryCreateModal}>Create Category</Button>
          </InnerHeaderRight>
        </InnerHeader>
        //list all categories
        <ul>
        ...
        //list all categories with subcategories and subsubcategories
        <ul>
          {categories.map((category) => (
            <li key={category._id}>
              {category.name}
        ...
        <Main>
          <Modal
            ...
            onHide={closeCategoryCreateModal}
            size="lg"
          ...
          >
            ...
            <ModalFooter>
              <Button onClick={closeCategoryCreateModal}>Close</Button>
            </ModalFooter>
          </Modal>
